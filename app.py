...
# Constants & Utilities
# ─────────────────────────────────────────────────────────────────────────────
START_DATE = pd.Timestamp('2025-07-01')  # simulation start
YEARS = 5
MONTHS = YEARS * 12
...
# ─────────────────────────────────────────────────────────────────────────────
# Sidebar: Scenario + Smoothing
# ─────────────────────────────────────────────────────────────────────────────
st.sidebar.title('Runway Simulator')
scenarios = load_scenarios()
choice = st.sidebar.selectbox('Load scenario', ['(new)'] + list(scenarios.keys()))
scenario_data = scenarios.get(choice, None)
...
with st.expander('How it works'):
    ...
    if prompt_path.exists():
        ...
        st.write("Upload an 'instructions.md' with usage details.")
...
new_init = ['United States','2025-08-01',0,0,0,1000,2000,0.3,0.1,1000,3000,10000,25000,50000,300,3,4,5,6,7,1_000_000,2_000_000,3_000_000,4_000_000,5_000_000]
...
prod_cols = ['Product','Start','Prep mo','Prep Tech/mo','Prep G&A/mo','CAC Mult','ARPU Mult',
             'Churn1 Mult','ChurnP Mult','CSC Mult','Ad1','Ad2','Ad3','Ad4','Ad5','M1','M2','M3','M4','M5','G1','G2','G3','G4','G5']
prod_init = ['AI Accounting','2025-07-01',6,2,20000,0.95,1.3,0.9,0.9,1.2,0.02,0.05,0.1,0.2,0.3,3,3,3,3,3,250000,500000,750000,1_000_000,1_500_000]
...
# Pack inputs
inputs = {'base': base_df.to_dict('list'),
          'new': new_df.to_dict('list'),
          'prod': prod_df.to_dict('list'),
          'eff': eff_df.to_dict('list'),
          'params': {'cash': cash_start,'cos': cos_pct,'hq_gna': hq_gna,
                     'hq_share': hq_share,'gna_share': gna_share,
                     'tech_units': tech_units,'tech_cost': tech_cost,
...
def simulate(inp):
    ...
    churn_post = np.concatenate([base['Churn Post'], newm['Churn Post']]).astype(float)
    ...
    for t in range(1,n):
        for i in range(m0+m1):
            ...
            for age in range(1,n): curr[age]=prev[age-1]*(1-(cy1[i] if age<12 else cp[i]))
    ...
    cac_eff=csc_eff=tc_eff=np.ones(n)
    for j in range(len(effp)):
        mrow=effp.loc[j,['CAC Mult','CSC Mult','TechCost Mult']].astype(float).values
        mask=eff_active[:,j]==1; cac_eff[mask]*=mrow[0]; csc_eff[mask]*=mrow[1]; tc_eff[mask]*=mrow[2]
    # containers
    ...
    gna_month=p['hq_gna']/12; cash_ser[0]=p['cash']; tech_av[0]=p['tech_units']
    for t in range(n):
        ...
        arpu, cac_, csc_ = base_arpu.copy(), base_cac.copy(), base_csc.copy()
        for j in range(len(prod)):
            frac=adopt[t,j]; r=prod.loc[j]
            arpu+=frac*(r['ARPU Mult']-1)*base_arpu
            cac_+=frac*(r['CAC Mult']-1)*base_cac
            csc_+=frac*(r['CSC Mult']-1)*base_csc
        cac_=cac_*cac_eff[t]; csc_=csc_*csc_eff[t]
        act=active[t]; nc=newc[t]
        rev_mkt.iloc[t]=act*arpu/12
        cos_df.iloc[t]=rev_mkt.iloc[t]*p['cos']
        acq_df.iloc[t]=nc*cac_
        serv_df.iloc[t]=act*csc_/12
        gna_ser[t]=gna_month
        if t>0:
            rev_prev, rev_cur = rev_mkt.iloc[t-1].sum(), rev_mkt.iloc[t].sum()
    ...
    costs_agg=pd.DataFrame({'CoS':cos_df.sum(axis=1),'Acquisition':acq_df.sum(axis=1),
                            'Servicing':serv_df.sum(axis=1),'G&A':gna_ser,
                            'Tech':tech_req*p['tech_cost']*tc_eff},index=idxs)
    tech_df=pd.DataFrame({'available':tech_av,'required':tech_req},index=idxs)
    denom=np.where(newc.sum(axis=1)==0,1,newc.sum(axis=1))
    metrics_df=pd.DataFrame({'CAC':acq_df.sum(axis=1)/denom,
                             ...
                             'Churn Post':(cp*active).sum(axis=1)/active.sum(axis=1)},index=idxs)
...
if st.button('Run Simulation'):
    ...
    rev_chart = alt.Chart(rev_df).mark_area().encode(
        x='index:T', y='Revenue:Q', color='Market:N'
    ).properties(width='container', height=300)
    st.altair_chart(rev_chart, use_container_width=True)

    # Costs by Market (smoothed)
    raw_cost = cos_df + acq_df + serv_df
    smoothed = raw_cost.rolling(window=smoothing_window, min_periods=1).mean()
    cost_df = smoothed[sel_mkt].reset_index().melt(id_vars='index', var_name='Market', value_name='Cost')
    cost_chart = alt.Chart(cost_df).mark_area().encode(
        x='index:T', y='Cost:Q', color='Market:N'
    ).properties(width='container', height=300)
    ...
    agg_chart = alt.Chart(agg_df).mark_area().encode(
        x='index:T', y='Amount:Q', color='Cost Type:N'
    ).properties(width='container', height=300)
    ...
    for j, prod_name in enumerate(prod_df['Product']):
        ...
        if prep>0: end_prep=min(start_idx+prep,len(arr)); arr[start_idx:end_prep]=row['Prep Tech/mo']
    ...
    tech_chart=alt.Chart(tech_long).mark_area().encode(
        x='index:T', y='Units:Q', color='Component:N'
    ).properties(width='container', height=300)
    avail_df=pd.DataFrame({'index':dates_idx, 'Available': tech_df['available']})
    avail_chart=alt.Chart(avail_df).mark_line(size=2).encode(x='index:T', y='Available:Q')
    st.altair_chart(alt.layer(tech_chart, avail_chart), use_container_width=True)
