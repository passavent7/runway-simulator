...
def simulate(inp):
    ...
    for i in range(m0): ys=base.loc[i,['New1','New2','New3','New4','New5']].values.astype(float)
        for yi in range(5): newc[yi*12:(yi+1)*12,i]=ys[yi]/12
    for i in range(m1):
        r=newm.loc[i]; idx=(pd.to_datetime(r['Start']).year-START_DATE.year)*12+pd.to_datetime(r['Start']).month-1
        prep=int(r['Prep mo']); ys=r[['New1','New2','New3','New4','New5']].values.astype(float)
    ...
    for j in range(len(prod)):
        ...
        prep=int(r['Prep mo']); ys=r[['Ad1','Ad2','Ad3','Ad4','Ad5']].values.astype(float)
        arr=np.zeros(n)
        for yi in range(5): s=idx+prep+yi*12; e=min(s+12,n)
    ...
    for j in range(len(effp)):
        mrow=effp.loc[j,['CAC Mult','CSC Mult','TechCost Mult']].values.astype(float)
        mask=eff_act[:,j]==1; cac_eff[mask]*=mrow[0]; csc_eff[mask]*=mrow[1]; tc_eff[mask]*=mrow[2]
    ...
    gna_ser=np.zeros(n); cash=np.zeros(n)
    gna_m=p['hq_gna']/12; cash[0]=p['cash']; tech_av[0]=p['tech_units']
    for t in range(n):
        ...
        if t>0:
            ...
            gna_m*=(1+p['gna_share']*gr); tech_av[t]=tech_av[t-1]*(1+p['tech_share']*gr)
        tech_req[t]=tech_av[t]*p['hq_share']
        total_rev=rev_mkt.iloc[t].sum(); total_cost=cos_df.iloc[t].sum()+acq_df.iloc[t].sum()+serv_df.iloc[t].sum()+gna_ser[t]+tech_req[t]*p['tech_cost']*tc_eff[t]
        cash[t]=cash[t-1]+total_rev-total_cost if t>0 else cash[0]
    costs_agg=pd.DataFrame({'CoS':cos_df.sum(axis=1),'Acq':acq_df.sum(axis=1),'Serv':serv_df.sum(axis=1),'G&A':gna_ser,'Tech':tech_req*p['tech_cost']*tc_eff},index=dates)
    metrics=pd.DataFrame({'CAC':acq_df.sum(axis=1)/(newc.sum(axis=1).replace(0,1)),'ARPU':rev_mkt.sum(axis=1)/active.sum(axis=1)*12,'Churn Y1':(cy1*active).sum(axis=1)/active.sum(axis=1),'Churn Post':(cp*active).sum(axis=1)/active.sum(axis=1)},index=dates)
    return rev_mkt, costs_agg, pd.DataFrame({'available':tech_av,'required':tech_req},index=dates), cash, metrics, cos_df, acq_df, serv_df
